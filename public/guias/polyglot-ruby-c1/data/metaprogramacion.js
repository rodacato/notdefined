...
(function (G) {
  ...
  D.fichas.push({
    ...
    snippet: [
      'class Config',
      '  CAMPOS = %i[host puerto tls]',
      '',
      '  CAMPOS.each do |campo|',
      '    define_method(campo) { @datos[campo] }',
      '  end',
      '',
      '  def initialize(**datos); @datos = datos; end',
      'end',
      '',
      ...
      'class Proxy',
      '  def initialize(destino); @destino = destino; end',
      '',
      '  def method_missing(nombre, *args, &blk)',
      '    return super unless @destino.respond_to?(nombre)',
      '    self.class.define_method(nombre) { |*a, &b| @destino.public_send(nombre, *a, &b) }',
      '    public_send(nombre, *args, &blk)',
      '  end',
      '',
      '  def respond_to_missing?(nombre, priv = false)',
      '    @destino.respond_to?(nombre, priv) || super',
      '  end',
      'end',
      '',
      'p = Proxy.new([3, 1, 2])',
  ...
  D.fichas.push({
    ...
    snippet: [
      ...
      'caja = Caja.new',
      '',
      'caja.instance_eval { def tapa; :singleton; end }',
      'caja.tapa                        # => :singleton',
      'caja.singleton_methods           # => [:tapa]',
      'Caja.instance_methods(false)     # => []',
      '',
      'Caja.class_eval { def tapa2; :instancia; end }',
      'Caja.new.tapa2                   # => :instancia',
      'Caja.instance_methods(false)     # => [:tapa2]',
      '',
      'Caja.instance_eval { def fabrica; :metodo_de_clase; end }',
      'Caja.fabrica                     # => :metodo_de_clase',
      '',
      'nombre = "desde el caller"',
      'caja.instance_exec(3) { |n| [self.class, n, nombre] }',
      '# => [Caja, 3, "desde el caller"]   # self cambió, el binding no'
    ].join('\n'),
