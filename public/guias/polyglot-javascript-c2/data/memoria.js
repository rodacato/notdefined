...
(function (G) {
  ...
  D.topics["garbage-collector"] = {
    ...
    snippet: [
      ...
      "const base = heap();",
      "let cache = [];",
      "for (let i = 0; i < 200000; i += 1) cache.push({ i, pad: 'xxxxxxxxxxxxxxxx' });",
      "const peak = heap();",
      "",
      "peak > base;   // => true",
      "",
      "cache = null;   // \u26a0\ufe0f suelta la referencia; NO libera nada todav\u00eda",
      "global.gc();",
      "",
      "heap() < peak;   // => true",
    ...
    callout: {
      ...
      cmd: "node --expose-gc -e \"const h=()=>process.memoryUsage().heapUsed;const b=h();let a=[];for(let i=0;i<2e5;i++)a.push({i});const p=h();a=null;global.gc();console.log('pico +'+Math.round((p-b)/1048576)+'MB, tras gc +'+Math.round((h()-b)/1048576)+'MB')\"",
      sale: "pico +10MB, tras gc +0MB · los MB varían",
    },
    recursos: [
  ...
  D.topics["shapes-inline-caches"] = {
    ...
    snippet: [
      ...
      "const a = { x: 1, y: 2 };",
      "const b = { x: 3, y: 4 };",
      "const reordered = { y: 4, x: 3 };",
      "const extended = { x: 1, y: 2, z: 3 };",
      "",
      "%HaveSameMap(a, b);           // => true",
      "%HaveSameMap(a, reordered);   // => false",
      "%HaveSameMap(a, extended);    // => false",
      "",
      "const built = {};",
      "built.x = 1;",
      "built.y = 2;",
    ...
    callout: {
      ...
      cmd: "node --allow-natives-syntax -e \"console.log(%HaveSameMap({x:1,y:2},{y:2,x:1}))\"",
      sale: "false",
    },
    recursos: [
      ...
      variants: [
        ...
        },
        {
          ...
          frames: [
            ...
            { code: ["getX({ x, y });", "getX({ y, x });", "getX({ x, y, z });", "getX({ z, x });", "getX({ w });"], phase: "Megam\u00f3rfico", shapes: ["A  {x, y}", "B  {y, x}", "C  {x, y, z}", "D  {z, x}", "E  {w}"],
              cap: "<strong>Megam\u00f3rfico</strong>: al pasar de 4 Shapes el IC se rinde y cae a la b\u00fasqueda gen\u00e9rica. Lento \u2014 esto es lo que evitas usando la misma forma." },
          ],
        },
      ],
    },
  ...
  D.topics["layout-memoria"] = {
    ...
    callout: {
      ...
      cmd: "node --allow-natives-syntax -e \"for (const n of [[42,42],[2**31,2147483648],[3.14,3.14]]) console.log(n[1], %IsSmi(n[0])?'Smi':'HeapNumber')\"",
      sale: "42 Smi · 2147483648 HeapNumber · 3.14 HeapNumber",
    },
    recursos: [
