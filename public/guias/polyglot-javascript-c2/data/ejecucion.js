...
(function (G) {
  ...
  D.topics["pipeline-ejecucion"] = {
    ...
    snippet: [
      "// node --print-bytecode --print-bytecode-filter=greet",
      "function greet(name) {",
      "  return 'Hello, ' + name;",
      "}",
      "",
    ...
    cuandoNo: "<p>Este es el único tema de la guía que <strong>no debe cambiar una sola línea</strong> de tu código. Es el mapa que hace legibles a las otras doce. Si sales de aquí queriendo «ayudarle al parser» —envolviendo funciones en paréntesis, partiendo archivos a mano— estás optimizando contra un motor que ya decidió solo.</p>",
    recursos: [
      ...
      variants: [{
        ...
        frames: [
          { phase: "C\u00f3digo fuente", codeCap: "snippet.js \u00b7 texto",
            code: ["function greet(name) {", "  return 'Hello, ' + name;", "}", "", "greet('world');"],
          ...
          { phase: "Tokens", codeCap: "flujo de tokens \u00b7 l\u00e9xico",
            code: ["FUNCTION  IDENT(greet)  (", "IDENT(name)  )  {", "RETURN  STRING('Hello, ')", "PLUS  IDENT(name)  }", "IDENT(greet) ( STRING('world') )"],
          ...
          { phase: "AST", codeCap: "\u00e1rbol sint\u00e1ctico \u00b7 estructura",
            code: ["FunctionDeclaration greet", "\u251C\u2500 params: [name]", "\u2514\u2500 body:", "   \u2514\u2500 Return", "      \u2514\u2500 BinaryExpr (+)", "         \u251C\u2500 'Hello, '", "         \u2514\u2500 name"],
          ...
          { phase: "Bytecode", codeCap: "Ignition bytecode \u00b7 int\u00e9rprete",
            code: ["LdaConstant [0]   // 'Hello, '", "Star0             // gu\u00e1rdalo en r0", "Ldar a0           // carga name", "Add r0, [0]       // r0 + acumulador", "Return"],
            cap: "<strong>Ignition</strong> emite bytecode compacto para su m\u00e1quina de registros. Esto es lo que se ejecuta primero, e incluye ranuras para recoger feedback de tipos.<br><span class=\"mono\" style=\"font-size:12px;color:var(--color-fg-faint)\">Salida literal de node --print-bytecode --print-bytecode-filter=greet \u2014 c\u00f3rrela t\u00fa.</span>" },
  ...
  D.topics["ignition-bytecode"] = {
    ...
    snippet: [
      "// node --print-bytecode --print-bytecode-filter=calc",
      "function calc(a, b) {",
      "  return a * b + 1;",
      "}",
      "",
    ...
    widget: {
      ...
      variants: [{
        ...
        code: ["Ldar a1", "Mul a0, [1]", "AddSmi [1], [0]", "Return"],
        frames: [
          ...
          { line: 1, phase: "Mul", regs: ["a0 \u00b7 3", "a1 \u00b7 4"], acc: ["12"], fb: ["[0] (vac\u00edo)", "[1] Number \u2713"],
            cap: "<strong>Mul</strong>: multiplica el acumulador por <span class=\"inline-code\">a0 (=3)</span>. Anota en el slot [1] que ambos operandos fueron n\u00fameros (Smi)." },
          { line: 2, phase: "AddSmi", regs: ["a0 \u00b7 3", "a1 \u00b7 4"], acc: ["13"], fb: ["[0] Number \u2713", "[1] Number \u2713"],
            cap: "<strong>AddSmi</strong>: suma la constante peque\u00f1a 1 al acumulador. Registra en el slot [0] \u2014 los slots no se llenan en el orden en que lees el c\u00f3digo." },
  ...
  D.topics["niveles-jit"] = {
    ...
    snippet: [
      ...
      "}",
      "",
      "const OPTIMIZED = 0b10000;",
      "const isOptimized = () => (%GetOptimizationStatus(add) & OPTIMIZED) !== 0;",
      "",
      "isOptimized();   // => false",
      "",
      "for (let i = 0; i < 200000; i += 1) add(i, 1);",
      "isOptimized();   // => true",
      "",
      "add('x', 'y');   // \u26a0\ufe0f un string donde TurboFan supuso n\u00fameros",
      "isOptimized();   // => false",
    ].join("\n"),
